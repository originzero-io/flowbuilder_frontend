...
import {
  addNewNode, addNewEdge, setNodes, setEdges, setNodeEnable, updateEdgePath,
...
export default function FlowEditor({ reactFlowWrapper }) {
  ...
  const {
    rotateAllPath, miniMapDisplay, edgeType, theme,
  } = flowGui;
  ...
  const updateRecentStatus = (type) => {
    const newList = nodeList.map((node) => (node.name === type ? { ...node, createdDate: Date.now() } : node));
